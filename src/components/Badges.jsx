...
export const checkBadgeUnlock = (stepsData, weatherData = {}, weatherEnabled = false) => {
  ...
  // ********************************************************
  // Default badges - always unlocked for all users
  // ********************************************************
  const defaultBadgeIds = [42, 43, 44]; // Blue, Orange, Yellow
  
  // Get user's creation date from localStorage
  let userCreatedAt = '04-07-2025'; // fallback
  try {
    ...
    console.warn('Could not parse userProfile from localStorage:', error);
  }
  
  defaultBadgeIds.forEach(badgeId => {
    const badge = badges.find(b => b.id === badgeId);
    if (badge) {
      unlockedBadges.push({
        ...badge,
        unlockDate: userCreatedAt
      });
    }
  });
